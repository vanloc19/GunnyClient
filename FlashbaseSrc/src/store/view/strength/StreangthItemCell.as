   ...
   public class StreangthItemCell extends StoreCell
   {
      ...
      override public function dragDrop(param1:DragEffect) : void
      {
         ...
         if(Boolean(_loc3_) && param1.action != DragEffect.SPLIT)
         {
            ...
            else if(_loc3_.CanStrengthen && this.isAdaptToStone(_loc3_))
            {
               ...
               if(_loc3_.StrengthenLevel == 9 && !SharedManager.Instance.isAffirm)
               {
                  SharedManager.Instance.isAffirm = true;
                  SharedManager.Instance.save();
                  _loc2_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("tank.game.GameViewBase.HintTitle"),LanguageMgr.GetTranslation("store.view.strength.clew"),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),false,true,true,LayerManager.ALPHA_BLOCKGOUND);
                  _loc2_.info.showCancel = false;
                  _loc2_.addEventListener(FrameEvent.RESPONSE,this._response);
               }
               SocketManager.Instance.out.sendMoveGoods(_loc3_.BagType,_loc3_.Place,BagInfo.STOREBAG,index,1);
               this._actionState = true;
               param1.action = DragEffect.NONE;
               DragManager.acceptDrag(this);
               this.reset();
            }
            else if(this.isAdaptToStone(_loc3_))
            {
            }
         }
      }
